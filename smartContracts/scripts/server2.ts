...
app.post("/face-data", async (req, res) => {
  try {
    ...
    if (!embedding) return res.status(400).json({ error: "embedding missing" });

    console.log(`[+] Received embedding length = ${embedding.length}`);

    // Float → BigInt (shift + scale)
    const embeddingBigInt: bigint[] = embedding.map((x: number) =>
      BigInt(Math.floor((x + SHIFT) * SCALE))
    );

    // BigInt → Fr[]
    const fieldElements: Fr[] = embeddingBigInt.map(
      (x) => new Fr(x % FIELD_PRIME)
    );

    // Poseidon2 Hash
    const hashFr: Fr = await api.poseidon2Hash(fieldElements);
    const hash: bigint = BigInt(hashFr.toString());
...
app.post("/face-verify", async (req, res) => {
  try {
    ...
    const embeddingStr = embedding.map((x: number) =>
      Math.floor((x + SHIFT) * SCALE).toString()
    );
    const enrolledStr = enrolled.map((x: number) =>
      Math.floor((x + SHIFT) * SCALE).toString()
    );
